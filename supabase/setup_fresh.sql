-- ══════════════════════════════════════════════════════════════════
-- Clark Cup: Complete setup for a FRESH Supabase project
-- Run this ONCE, top to bottom, in the Supabase SQL Editor.
--
-- Replaces the old two-step process (schema.sql then
-- migration_historical.sql), which could not run on an empty project
-- and left behind a signup-breaking trigger. See NOTES at the bottom.
-- ══════════════════════════════════════════════════════════════════
...
-- FIX 1: lets a new signup claim their pre-seeded placeholder profile.
-- Without this, claiming silently updates zero rows (the policy above
-- requires auth_user_id = auth.uid(), but a placeholder's is NULL) and
-- the new user ends up with no profile at all.
CREATE POLICY "Users can claim an unclaimed profile"
...
-- FIX 2: deliberately NO handle_new_user() trigger on auth.users.
-- The app creates and claims profiles itself in app/(auth)/signup.tsx.
-- A trigger that auto-inserts a profile would create a second row with
-- auth_user_id NULL, and the claim lookup in signup.tsx uses .single(),
-- which errors when two unclaimed rows share a username -- so signup
-- would fail outright for every family member with historical data.
...
-- One per family member, holding their historical points until they
-- sign up and claim it. Idempotent: skips a username already present.
...
-- ══════════════════════════════════════════════════════════════════
-- NOTES
-- Family members sign up with the invite code and their display name
-- (Kyle, Kari, Mom, Kris, Kelly) to claim the matching profile above,
-- inheriting its historical points. Names must match exactly.
-- ══════════════════════════════════════════════════════════════════
